...
def prune_wanda_sp(model, tokenizer, nsamples, seqlen, seed, pruning_ratio, device=torch.device("cuda:0")):
            ...
            if pruning_ratio_layer > 0:
                for name in subset:
                    ...
                    if name == layer_name['m1']:
                        W_metric = W_metric.mean(axis=0).reshape(-1, 896).sum(dim=1)  # importance score of each head
    ...
    model.config.hidden_size = layers[0].self_attn.hidden_size
    model.config.num_attention_heads = layers[0].self_attn.num_heads
    model.config.num_key_value_heads = layers[0].self_attn.num_key_value_heads
    torch.cuda.empty_cache()
...
def compress(layer, attn_mask, mlp_mask, attn_mean_inp, mlp_mean_inp, device, bias=True):
    ...
    if attn_mask is not None:
        retain_heads = torch.count_nonzero(attn_mask)  # k,v ' heads
        layer.self_attn.num_key_value_heads = retain_heads.item()
        attn_mask = attn_mask.repeat_interleave(128)

        # layer.self_attn.k_proj and layer.self_attn.v_proj prune
        # 剪枝 K_proj 层的权重
        ...
        # 剪枝 Q_proj 层的权重
        attn_mask = attn_mask.repeat_interleave(7) # 把prune k，v的attn_mask重复七次得到prune q，o的ATTN_mask
        ...
        # 假设 retain_heads 是一个张量，表示保留的注意力头数量, q,o de head retain_head of k *7
        retain_heads = retain_heads * 7
